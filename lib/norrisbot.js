...
var NorrisBot = function Constructor(settings) {
    ...
    this.settings.name = this.settings.name || 'chucknorris';
...
NorrisBot.prototype._onMessage = function (message) {
    console.log("On Message:" + util.inspect(message, false, null));
    if (this._isChatMessage(message) &&
        this._isChannelConversation(message) &&
        !this._isFromNorrisBot(message) &&
        this._isMentioningChuckNorris(message)
    ) {
        console.log("Try to reply with Joke: ");
...
NorrisBot.prototype._replyWithRandomJoke = function (originalMessage) {
    console.log("In Random Joke and originalMessage is " + originalMessage);
    var self = this;
    console.log("Self is: " + util.inspect(self, false, null));
    self.db.get('SELECT id, joke FROM jokes ORDER BY used ASC, RANDOM() LIMIT 1', function (err, record) {
        console.log("Record is: " + util.inspect(record, false, null));
        if (err) {
...
NorrisBot.prototype._isFromNorrisBot = function (message) {
    ...
    console.log("isFromNorrisBot User is:" + util.inspect(this, false, null))
